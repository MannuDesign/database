...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows === 1) {
        ...
        $heading = $row['headingfeq'];
        $description = $row['descriptionfeq'];
    } else {
        echo "<script>alert('FAQ not found!'); window.location.href='view-faq.php';</script>";
        exit();
    }
}
...
if (isset($_POST['update'])) {
    $heading = $_POST['headingfeq'];
    $description = $_POST['descriptionfeq'];

    $sql = "UPDATE faq SET headingfeq='$heading', descriptionfeq='$description' WHERE id=$id";

    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('FAQ updated successfully!'); window.location.href='view-faq.php';</script>";
    } else {
                                                    ...
                                                    <div class="row g-3">
                                                        <div class="col-lg-12">
                                                            <div class="mt-3">
                                                                ...
                                                                <input type="text" name="headingfeq"
                                                                    class="form-control" placeholder="Enter FAQ heading"
                                                                    value="<?php echo isset($heading) ? htmlspecialchars($heading) : ''; ?>"
                                                                    required />
                                                            </div>

                                                            <div class="mt-3">
                                                                <label class="form-label">FAQ Description</label>
                                                                <textarea name="descriptionfeq" class="form-control"
                                                                    rows="4" placeholder="Enter FAQ description..."
                                                                    required><?php echo isset($description) ? htmlspecialchars($description) : ''; ?></textarea>
                                                            </div>

                                                            <div class="mt-4">
                                                                <button class="btn btn-primary" name="update"
                                                                    type="submit">Update FAQ</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
